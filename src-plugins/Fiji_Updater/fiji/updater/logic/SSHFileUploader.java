...
public class SSHFileUploader extends FileUploader {
	protected static class SSHHost {
		protected String hostName;
		protected int hostKeyType;
		protected byte[] hostKey;

		public SSHHost(String hostName, int hostKeyType, String hostKey) {
			this(hostName, hostKeyType, getBytes(hostKey));
		}

		public SSHHost(String hostName, int hostKeyType, byte[] hostKey) {
	...
	protected final static Map<String, SSHHost> sshHosts;

	static {
		sshHosts = new HashMap<String, SSHHost>();
		sshHosts.put(Updater.SSH_HOST, new SSHHost(Updater.SSH_HOST, HostKey.SSHRSA,
	...
	public SSHFileUploader(String username, String password,
			String sshHost, String uploadDirectory) throws JSchException {
		super(uploadDirectory);

		SSHHost host = sshHosts.get(sshHost);
		if (host == null)
			throw new RuntimeException("TODO: get host key (and save to prefs), initialize server if necessary");

		JSch jsch = new JSch();
		HostKey hostKeyObject = new HostKey(sshHost, host.hostKeyType, host.hostKey);
	...
	protected static byte[] getBytes(String hostKey) {
		ByteArrayOutputStream out = new ByteArrayOutputStream();
		byte previousNybble = -1;
		for (char c : hostKey.toCharArray()) {
			if (Character.isWhitespace(c))
				continue;
			if (previousNybble < 0) {
				previousNybble = (byte)fromHex(c);
				continue;
			}
			out.write((previousNybble << 4) | fromHex(c));
			previousNybble = -1;
		}
		if (previousNybble >= 0)
			throw new RuntimeException("Odd number of hex digits: " + hostKey);
		return out.toByteArray();
	}

	protected static int fromHex(char c) {
		if (c >= '0' && c <= '9')
			return c - '0';
		if (c >= 'a' && c <= 'f')
			return c - 'a' + 10;
	...
	protected static String toHexString(byte[] array) {
		StringBuffer buffer = new StringBuffer();
		for (byte b : array)
			buffer.append(' ').append(Integer.toHexString((b >> 4) & 0xf))
