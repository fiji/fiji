...
%
% NOTES:
%   To use the internal viewer, follow the installation instructions below.
%
...
%   You need to modify the MATLAB file 'classpath.txt' to use the Java applet View5D.jar:
...
if ~any(strcmp(whe,{'direct','extern'}))
  error('MODE string should be ''direct'' or ''extern''.')
end
...
if isa(in,'dip_image_array')
  ...
  if ~isvector(in)
     error('No vector/spectral image.');
  end
  if length(sz)>4
...
%------------------------------------------------------------------
function out=view5d_image_direct(in)
