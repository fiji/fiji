 ...
 *
 */
public class SpotCollectionHashSet implements Iterable<Spot>, SortedMap<Integer, Set<Spot>>, MultiThreaded  {
	 ...
	 * comparator, if any).
	 */
	public SpotCollectionHashSet(ConcurrentSkipListMap<Integer, Set<Spot>> content) {
	 ...
	 */
	public SpotCollectionHashSet() {
		this(new ConcurrentSkipListMap<Integer, Set<Spot>>());
	}
	 ...
	 * new collection.
	 */
	public SpotCollectionHashSet subset(Collection<Spot> spots) {
		SpotCollectionHashSet newCollection = new SpotCollectionHashSet();
		Integer frame;
		for(Spot spot : spots) {
			frame = getFrame(spot);
			if (null == frame)
...
//		spot.putFeature(Spot.FRAME, frame);
	 ...
	 */
	public final SpotCollectionHashSet filter(final FeatureFilter featurefilter) {
		final SpotCollectionHashSet selectedSpots = new SpotCollectionHashSet();
			...
			threads[ithread] = new Thread("TrackMate filtering spot thread "+ithread) {
				...
				public void run() {
					...
					for (int i = ai.getAndIncrement(); i < keys.length; i = ai.getAndIncrement()) {
						...
						spotToKeep = new HashSet<Spot>(spotThisFrame);
						spotToRemove = new HashSet<Spot>(spotThisFrame.size());
	 ...
	 */
	public final SpotCollectionHashSet filter(final Collection<FeatureFilter> filters) {
		SpotCollectionHashSet selectedSpots = new SpotCollectionHashSet();
		selectedSpots.setNumThreads(numThreads);
		Set<Spot> spotThisFrame, spotToRemove;
		HashSet<Spot> spotToKeep;
		Double val, tval;	
	 ...
	 */
	public final HashSet<Spot> getNClosestSpots(final Spot location, final int frame, int n) {
		final Set<Spot> spots = content.get(frame);
		...
		final HashSet<Spot> selectedSpots = new HashSet<Spot>(n);
	...
	public int getNSpots(int key) {
		Set<Spot> spots = content.get(key);
		if (null == spots)
			return 0;
		else
			return spots.size();
	 ...
	 * they belong to).
	 */
	public final HashSet<Spot> getAllSpots() {
		HashSet<Spot> allSpots = new HashSet<Spot>(getNSpots()); 
		for(Set<Spot> spots : content.values())
	 ...
	 */
	@Override
	public Set<Spot> get(Object key) {
		Set<Spot> spots = content.get(key);
		if (null == spots) {
	...
	@Override
	public Set<Spot> put(Integer key, Set<Spot> value) {
//		if (key == null)
//			return null;
//		for (Spot spot : value) {
...
//		}
	...
	@Override
	public Set<Spot> remove(Object key) {
		return content.remove(key);
	}

	@Override
	public void putAll(Map<? extends Integer, ? extends Set<Spot>> map) {
	...
	@Override
	public SortedMap<Integer, Set<Spot>> subMap(Integer fromKey, Integer toKey) {
		return content.subMap(fromKey, toKey);
	}

	@Override
	public SortedMap<Integer, Set<Spot>> headMap(Integer toKey) {
		return content.headMap(toKey);
	}

	@Override
	public SortedMap<Integer, Set<Spot>> tailMap(Integer fromKey) {
	...
	@Override
	public Collection<Set<Spot>> values() {
		return content.values();
	}

	@Override
	public Set<java.util.Map.Entry<Integer, Set<Spot>>> entrySet() {
