	...
	public void paintShape(mxGraphics2DCanvas canvas, String text, mxCellState state, Map<String, Object> style) {
		...
		if (g.getClipBounds() == null || g.getClipBounds().intersects(rect))
		{
			...
			if (isEdgeLabel) {
				...
				double dx1 = points.get(0).getX();
				double dy1 = points.get(0).getY();
				double dx2 = points.get(1).getX();
				double dy2 = points.get(1).getY();
				g.rotate(Math.atan2(dy2-dy1, dx2-dx1), x + w / 2, y + h / 2); // FIX ME shift in x and y, and also change by pi when upside down
			} else if (!horizontal)	{
