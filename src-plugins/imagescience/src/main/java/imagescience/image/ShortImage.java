...
/** An image containing up to 5D elements of type {@code short}. Element values are taken in the unsigned sense and therefore the {@code get} methods always return numbers in the range {@code [0,65535]}. Conversion of floating-point numbers in the {@code set} methods is done by clipping to the range [0,65535] and rounding to the nearest {@code short} value. */
public class ShortImage extends Image {
	...
	*/
	public ShortImage(final Dimensions dims) {
		
		super(dims.duplicate());
		elements = new short[dims.c][dims.t][dims.z][dims.y*dims.x];
	}
	...
	/** Wrapper constructor.
		...
		@exception IllegalArgumentException if the image elements of {@code imageplus} are not of type {@code short}.
		
		@exception NullPointerException if {@code imageplus} is {@code null}.
	*/
	public ShortImage(final ImagePlus imageplus) {
		...
		if (!(slices[0] instanceof short[])) throw new IllegalArgumentException("Wrong input type");
		
		elements = new short[dims.c][dims.t][dims.z][];
		for (int t=0; t<dims.t; ++t)
			for (int z=0; z<dims.z; ++z)
				for (int c=0; c<dims.c; ++c)
					elements[c][t][z] = (short[])slices[t*dims.z*dims.c + z*dims.c + c];
	}
	...
	*/
	public ShortImage(final Image image) {
	...
	*/
	public ShortImage(final Image image, final Borders borders) {
	...
	public String type() { return "imagescience.image.ShortImage"; }
	
	public int memory() { return 2*elements(); }
	
	public Image duplicate() {
		
		final ShortImage dupimg = new ShortImage(dims);
		...
		for (int c=0; c<dims.c; ++c)
			for (int t=0; t<dims.t; ++t)
				for (int z=0; z<dims.z; ++z) {
					final short[] thiselms = elements[c][t][z];
					final short[] dupelms = dupimg.elements[c][t][z];
	...
	public double get(final Coordinates coords) {
		...
		return 0xFFFF&elements[coords.c][coords.t][coords.z][dims.x*coords.y + coords.x];
	}
	...
	public void get(final Coordinates coords, final double[] values) {
		...
		switch (axes) {
			case Axes.X: {
				...
				final short[] elms = elements[coords.c][coords.t][coords.z];
				for (int x=exstart, ex=coords.y*dims.x+exstart, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
					values[vx] = 0xFFFF&elms[ex];
				break;
			}
			case Axes.Y: {
				int vystart = 0;
			...
			case Axes.Z: {
				...
				final short[][] elms = elements[coords.c][coords.t];
				for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
					values[vz] = 0xFFFF&elms[ez][exy];
				break;
			}
			case Axes.T: {
				int vtstart = 0;
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					values[vt] = 0xFFFF&elements[coords.c][et][coords.z][exy];
				break;
			}
			case Axes.C: {
				int vcstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					values[vc] = 0xFFFF&elements[ec][coords.t][coords.z][exy];
				break;
			}
			default:
			...
			case Axes.ZX: {
				...
				for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
					for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
						values[vz][vx] = 0xFFFF&elms[ez][ex];
				break;
			}
			case Axes.ZY: {
				int vystart = 0;
				...
				for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
					for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
						values[vz][vy] = 0xFFFF&elms[ez][ey];
				break;
			}
			case Axes.TX: {
				int vxstart = 0;
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
						values[vt][vx] = 0xFFFF&elements[coords.c][et][coords.z][ex];
				break;
			}
			case Axes.TY: {
				int vystart = 0;
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
						values[vt][vy] = 0xFFFF&elements[coords.c][et][coords.z][ey];
				break;
			}
			case Axes.TZ: {
				int vzstart = 0;
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						values[vt][vz] = 0xFFFF&elements[coords.c][et][ez][exy];
				break;
			}
			case Axes.CX: {
				int vxstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
						values[vc][vx] = 0xFFFF&elements[ec][coords.t][coords.z][ex];
				break;
			}
			case Axes.CY: {
				int vystart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
						values[vc][vy] = 0xFFFF&elements[ec][coords.t][coords.z][ey];
				break;
			}
			case Axes.CZ: {
				int vzstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						values[vc][vz] = 0xFFFF&elements[ec][coords.t][ez][exy];
				break;
			}
			case Axes.CT: {
				int vtstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						values[vc][vt] = 0xFFFF&elements[ec][et][coords.z][exy];
				break;
			}
			default:
				throw new IllegalStateException("Wrong number of active axes");
		...
		switch (axes) {
			case Axes.ZYX: {
				...
				for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
					for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
						for (int x=exstart, ex=ey, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
							values[vz][vy][vx] = 0xFFFF&elms[ez][ex];
				break;
			}
			case Axes.TYX: {
				int vxstart = 0;
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
						for (int x=exstart, ex=ey, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
							values[vt][vy][vx] = 0xFFFF&elements[coords.c][et][coords.z][ex];
				break;
			}
			case Axes.CYX: {
				int vxstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
						for (int x=exstart, ex=ey, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
							values[vc][vy][vx] = 0xFFFF&elements[ec][coords.t][coords.z][ex];
				break;
			}
			case Axes.TZX: {
				int vxstart = 0;
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
							values[vt][vz][vx] = 0xFFFF&elements[coords.c][et][ez][ex];
				break;
			}
			case Axes.CZX: {
				int vxstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
							values[vc][vz][vx] = 0xFFFF&elements[ec][coords.t][ez][ex];
				break;
			}
			case Axes.CTX: {
				int vxstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
							values[vc][vt][vx] = 0xFFFF&elements[ec][et][coords.z][ex];
				break;
			}
			case Axes.TZY: {
				int vystart = 0;
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
							values[vt][vz][vy] = 0xFFFF&elements[coords.c][et][ez][ey];
				break;
			}
			case Axes.CZY: {
				int vystart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
							values[vc][vz][vy] = 0xFFFF&elements[ec][coords.t][ez][ey];
				break;
			}
			case Axes.CTY: {
				int vystart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
							values[vc][vt][vy] = 0xFFFF&elements[ec][et][coords.z][ey];
				break;
			}
			case Axes.CTZ: {
				int vzstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
							values[vc][vt][vz] = 0xFFFF&elements[ec][et][ez][exy];
				break;
			}
			default:
				throw new IllegalStateException("Wrong number of active axes");
			...
			case Axes.TZYX: {
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
							for (int x=exstart, ex=ey, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
								values[vt][vz][vy][vx] = 0xFFFF&elements[coords.c][et][ez][ex];
				break;
			}
			case Axes.CZYX: {
				int vxstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
							for (int x=exstart, ex=ey, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
								values[vc][vz][vy][vx] = 0xFFFF&elements[ec][coords.t][ez][ex];
				break;
			}
			case Axes.CTYX: {
				int vxstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
							for (int x=exstart, ex=ey, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
								values[vc][vt][vy][vx] = 0xFFFF&elements[ec][et][coords.z][ex];
				break;
			}
			case Axes.CTZX: {
				int vxstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
							for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
								values[vc][vt][vz][vx] = 0xFFFF&elements[ec][et][ez][ex];
				break;
			}
			case Axes.CTZY: {
				int vystart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
							for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
								values[vc][vt][vz][vy] = 0xFFFF&elements[ec][et][ez][ey];
				break;
			}
			default:
				throw new IllegalStateException("Wrong number of active axes");
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
							for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
								for (int x=exstart, ex=ey, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
									values[vc][vt][vz][vy][vx] = 0xFFFF&elements[ec][et][ez][ex];
				break;
			}
			default:
				throw new IllegalStateException("Wrong number of active axes");
	...
	public void set(final double value) {
		...
		final int dimsxy = dims.x*dims.y;
		final short svalue = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
		for (int c=0; c<dims.c; ++c)
			for (int t=0; t<dims.t; ++t)
				for (int z=0; z<dims.z; ++z)
					for (int xy=0; xy<dimsxy; ++xy)
						elements[c][t][z][xy] = svalue;
	}
	...
	public void set(final Coordinates coords, final double value) {
		...
		elements[coords.c][coords.t][coords.z][coords.y*dims.x + coords.x] =
			(short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
	}
		...
		switch (axes) {
			case Axes.X: {
				...
				for (int x=exstart, ex=coords.y*dims.x+exstart, vx=vxstart; x<exstop; ++x, ++ex, ++vx) {
					...
					elms[ex] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
				}
				break;
			}
			case Axes.Y: {
				...
				for (int y=eystart, ey=eystart*dims.x+coords.x, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy) {
					...
					elms[ey] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
				}
				break;
			}
			case Axes.Z: {
				...
				for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz) {
					...
					elms[ez][exy] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
				}
				break;
			}
			case Axes.T: {
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt) {
					...
					elements[coords.c][et][coords.z][exy] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
				}
				break;
			}
			case Axes.C: {
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc) {
					...
					elements[ec][coords.t][coords.z][exy] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
				}
				break;
			}
			default:
			...
			case Axes.ZX: {
				...
				for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
					for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx) {
						...
						elms[ez][ex] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
					}
				break;
			}
			case Axes.TX: {
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx) {
						...
						elements[coords.c][et][coords.z][ex] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
					}
				break;
			}
			case Axes.ZY: {
				...
				for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
					for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy) {
						...
						elms[ez][ey] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
					}
				break;
			}
			case Axes.TY: {
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy) {
						...
						elements[coords.c][et][coords.z][ey] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
					}
				break;
			}
			case Axes.TZ: {
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz) {
						...
						elements[coords.c][et][ez][exy] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
					}
				break;
			}
			case Axes.CX: {
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx) {
						...
						elements[ec][coords.t][coords.z][ex] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
					}
				break;
			}
			case Axes.CY: {
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy) {
						...
						elements[ec][coords.t][coords.z][ey] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
					}
				break;
			}
			case Axes.CZ: {
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz) {
						...
						elements[ec][coords.t][ez][exy] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
					}
				break;
			}
			case Axes.CT: {
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt) {
						...
						elements[ec][et][coords.z][exy] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
					}
				break;
			}
			default:
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx) {
							...
							elements[coords.c][et][ez][ex] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
						}
				break;
			}
			case Axes.CZX: {
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx) {
							...
							elements[ec][coords.t][ez][ex] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
						}
				break;
			}
			case Axes.CTX: {
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx) {
							...
							elements[ec][et][coords.z][ex] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
						}
				break;
			}
			case Axes.TZY: {
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy) {
							...
							elements[coords.c][et][ez][ey] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
						}
				break;
			}
			case Axes.CZY: {
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy) {
							...
							elements[ec][coords.t][ez][ey] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
						}
				break;
			}
			case Axes.CTY: {
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy) {
							...
							elements[ec][et][coords.z][ey] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
						}
				break;
			}
			case Axes.CTZ: {
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz) {
							...
							elements[ec][et][ez][exy] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
						}
				break;
			}
			default:
					...
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
							for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx) {
								...
								elements[ec][et][ez][ex] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
							}
				break;
			}
			case Axes.CTZY: {
					...
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
							for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy) {
								...
								elements[ec][et][ez][ey] = (short)FMath.round((value>0xFFFF)?0xFFFF:((value<0)?0:value));
							}
				break;
			}
			default:
