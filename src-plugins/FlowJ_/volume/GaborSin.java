 ...
 */
public class GaborSin extends Kernel1D
{
    /**
     ...
     */
    public GaborSin(double sigma, double w)
    {
        if (sigma != 0)
        {
            ...
            for (int l = -halfwidth; l <= halfwidth; l++)
		            k[l+halfwidth-sw*l] = 1 / (Math.sqrt(2*Math.PI)*sigma) * Math.sin(1*l*w) * Math.exp(-(l*l) / (2*sigma*sigma));
        }
        else
