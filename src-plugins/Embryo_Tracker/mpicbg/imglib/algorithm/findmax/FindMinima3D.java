...
public class FindMinima3D<T extends RealType<T>> implements Algorithm, Benchmark, LocalMaximaFinder
{
	...
	final protected ArrayList< double[] > minima = new ArrayList< double[] >();	// an array list which holds the coordinates of the maxima found in the image.
	 ...
	 */
	public FindMinima3D( final Image<T> image, final OutOfBoundsStrategyFactory<T> outOfBoundsFactory, boolean allowEdgeMax)
	{
		this.image = image;
	...
	public boolean process()
	{
		...
		boolean isMin;													// stores whether our lake/connected component is a local maxima or not.
		...
		double averagedMinPos[] = new double[3];								// holds the averaged coordinates if our lake was a local maxima.
		...
		while(curr.hasNext()) { 
			...
			while ((nextCoords = toSearch.poll()) != null) {
				...
				while(neighbors.hasNext()) {
					...
					// Case 1: neighbor's value is strictly less than ours, so ours cannot be a local minimum.
					if (neighborValue.compareTo(currentValue) < 0) {
						isMin = false;
					}
					
					// Case 2: neighbor's value is strictly equal to ours, which means we could still be at a minimum, but the min value is a blob, not just a single point. We must check the area.
			...
			if (isMin) {  // If isMax is still true, then our lake/connected component is a local maximum, so find the averaged point in the center.
				if (searched.size() > 0) {
					averagedMinPos = findAveragePosition(searched);
					minima.add(averagedMinPos);
				}
	 ...
	 * @return
	 */
	public ArrayList< double[] > getLocalMaxima() { return minima;	}
	 ...
	 * @return
	 */
	final static protected double[] findAveragePosition(final LinkedList < int[] > searched) {
		int count = 0;
