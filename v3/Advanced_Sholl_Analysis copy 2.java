...
public class Advanced_Sholl_Analysis implements PlugIn {
    ...
    static public double[] analyze3D(int xcenter, int ycenter, int zcenter,
        ...
        int imgArea = (bounds[2]-bounds[0]+1) * (bounds[3]-bounds[1]+1);

        //  Create an array to hold the pixels on each slice
        int[] values = new int[imgArea];
        int[][] points = new int[imgArea][2];
        ...
        for (int i = 0; i < nspheres; i++) {
            ...
            data[i] = countTargetGroups(values, points, ip);
        }
    return data;
    }
