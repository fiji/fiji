...
public class Advanced_Sholl_Analysis_3D implements PlugIn {
    ...
    public void run(String arg) {
        ...
        if (!IS_3D) {
            gd.addNumericField("Radius_span:", spanWidth, 2, 9, Unit);
            //gd.addSlider("Samples per radius:", 1, 20, 1);
            gd.addChoice("Span_type:", BIN_TYPES, BIN_TYPES[binChoice]);
        } else
            SHOLL_TYPES[SHOLL_NS] = SHOLL_TYPES[SHOLL_NS].replaceAll("Area", "Volume");
    ...
    static public double[] analyze3D(int xcenter, int ycenter, int zcenter,
        ...
        //  Create an array to hold the pixels of the entire stack
        int volumesize = (bounds[2]-bounds[0]+1) * (bounds[3]-bounds[1]+1) * depth;
        int[][][] points = new int[volumesize][2][2];
        ...
        for (int i = 0; i < nspheres; i++) {
            ...
            data[i] = count3DGroups(points, zcount, 1.5);
            zcount = 0;
 ...
 static public int count3DGroups(int[][][] points, int stopindex, double threshold) {

        double distance;
        int i, j, k, dx, dy, dz, groups, len;

        len = (stopindex<0) ? points.length : stopindex;

        int col1 = points[0].length;
        int col2 = points[1].length;
        ...
        for (i = 1; i < len; i++) {
            for (j = 1; j < col1; j++) {
               for (k = 1; k < col2; k++) {
                    ...
                    // there are lots of other possibilities. This is just
                    // a start
                    dx = points[i][0][0] - points[i-1][0][0];
                    dy = points[j][1][0] - points[j-1][1][0];
                    dz = points[k][0][1] - points[k-1][0][1];

                    distance = Math.sqrt(dx*dx + dy*dy + dz*dz);

                    // Should these two points be in the same group?
                    if ( (distance <= threshold) ) groups++;

                }
            }
        }
                //IJ.log("groups"+ groups +", len:"+ len);
        return (len-groups);
    ...
    public ImagePlus makeMask(ImagePlus img, String ttl, double[] values,
        ...
        // No point in continuing if image is no longer available, or if arrays
        // do not match
        ImageProcessor ip  = img.getProcessor();
        if ( ip==null || drawSteps > xvalues.length ) return null;
        ...
        ImagePlus img2 = IJ.createImage("Sholl mask [" +
            SHOLL_TYPES[shollChoice] + "] for " + ttl, "32-bit black", width,
            height, depth);
