...
public class Advanced_Sholl_Analysis implements PlugIn {
    ...
    public void run(String arg) {
        ...
        if (restrict) { // if restrict, chord and chordAngle are already defined
    ...
    static public double[] analyze3D(int xcenter, int ycenter, int zcenter,
		...
		voxels = (int)Math.round( (maxX-minX+1) * (maxY-minY+1) * (depth) );
        ...
        for (int s = 0; s < nspheres; s++) {
            ...
            for( int z = 1; z <= depth; ++z ) {
				...
				IJ.showProgress(z*s, depth*nspheres);
    ...
    public ImagePlus makeMask(ImagePlus img, String ttl, double[] values,
        ...
        // prepare mask image: A 32-bit img, so that it can hold any type of
        // transformed Sholl value
        String mtitle = "Sholl mask ["+ SHOLL_TYPES[shollChoice] +"] for "+ ttl;
        ImagePlus img2;
        ImageProcessor maskbp = new FloatProcessor(width, height);
long startmask = System.currentTimeMillis();
                    ...
                    for (k = 0; k < points.length; k++)
                        for (l = 0; l < points[k].length; l++)
                            if (ip.get(points[k][0], points[k][1]) != 0)
                                maskbp.putPixelValue(points[k][0], points[k][1], values[i]);

                }
            }

            img2 = new ImagePlus(mtitle, maskbp);
                        ...
                        for ( int y = minY; y < maxY; ++y ) {
                            ...
                            for ( int x= minX; x < maxX; ++x ) {
                                ...
                                if ( Math.abs(distanceToRadius) < 1 ) { //<=0.5
                                    if (ip.get(x, y)!=0 )
                                        maskbp.putPixelValue(x, y, values[i]);
                                }
                            }
                        }
                    }

                maskstack.addSlice("", maskbp);

                }

                img2 = new ImagePlus(mtitle, maskstack);
...
IJ.log(""+ IJ.d2s((System.currentTimeMillis()-startmask)/1000.0, 3) +"s. ");
