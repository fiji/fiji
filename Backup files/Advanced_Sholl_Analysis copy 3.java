 ...
 *
 * Performs Sholl Analysis on binary images of arbors previously traced or segmented.
...
public class Advanced_Sholl_Analysis implements PlugIn {
    ...
    /* Plugin Information */
    public static final String VERSION = "2.2c";
    ...
    private static final String[] SHOLL_TYPES = 
    		{"Intersections", "Inters./Area", "Semi-Log", "Log-Log"};
    ...
    private static final String[] DEGREES =
    		{"4th degree", "5th degree", "6th degree", "7th degree", "8th degree"};
    ...
    private static double  circWidth   = 2;
    ...
    public void run(String arg) {
        ...
        ImagePlus img = IJ.getImage();          // Get current image
        ImageProcessor ip = img.getProcessor(); // Get the ImageProcessor for the image
        ...
        if (!IJ.macroRunning() && !((roi != null && roi.getType() == Roi.LINE) ||
            ...
            roi = img.getRoi(); // Get new ROI, in case it has changed
        ...
        if (roi != null && roi.getType() == Roi.LINE) {
            ...
            endRadius = line.getRawLength()*pxSize; // Get the length of the line
        ...
        // Create the plugin dialog and keep presenting it until the user gets
        // it right (unless running from a macro)
        ...
        // Exit if there are no samples to be taken
        if (size==1) {                
        ...
        double[] yvalues = analyze(x, y, radii, (int)(circWidth/pxSize), binChoice, ip);

        IJ.showStatus("Preparing Results...");

        // Display the analysis and return transformed data
        double[] grays = plotValues(img.getTitle(), shollChoice, radii, xvalues, yvalues, x, y);
        ...
        if (makeMask) {
            ...
            ImagePlus img2 = IJ.createImage(SHOLL_TYPES[shollChoice]+" mask for "+ img.getTitle(),
            ...
            for (i = 0; i < drawSteps; i++) {
                ...
                drawRadius = (int)Math.round( (startRadius/pxSize) + (i*drawWidth) - drawWidth );
 ...
 * for each */
    static public double[] analyze(int x, int y, int[] radii, int binsize, int bintype,
        ...
        for (i = 0; i < size; i++) {
            ...
            IJ.showStatus("Sampling radius "+ i +"/"+ size +" w/ integration of "+ binsize +" measurements");
            //IJ.showProgress(i*binsize, size*binsize);
...
/* Create the Sholl plot */
    static public double[] plotValues(String ttl, int mthd, int[] r, double[] xpoints,
        ...
        // get non-zero values and calculate "log intersections". The latter will
        ...
        if (!fitCurve) {
            ...
            if (mthd!=SHOLL_SLOG) parameters = cf.getParams();
            ...
            if (mthd==SHOLL_N) {
                ...
                // We'll iterate around the index of highest fitted value to retrive
                ...
                for (i=0; i<iterations; i++) {
                    ...
                    if (cvTmp>cv) { cv = cvTmp; cr = crTmp; }
                }
                rt.addValue("Critical value", cv);
                rt.addValue("Critical radius", cr);
                
                // Calculate mv, the mean value of the fitted Sholl function. This
                // can be done assuming that the mean value is the height of a rectangle
                // that has the width of (NonZeroEndRadius - NonZeroStartRadius) and the
                // same area of the area under the fitted curve on that discrete interval
                for (i=0; i<parameters.length-1; i++ ) //-1?
                    mv = mv + ( (parameters[i]/(i+1)) * (Math.pow(xScale[1]-xScale[0], i)) );
                ...
                // Add calculated parameters to plot label and adjust its position
                labelPos = 0.99;
                label += "\nCv= " + IJ.d2s(cv, 2)
                      +  "\nCr= " + IJ.d2s(cr, 2)
                      +  "\nMv= " + IJ.d2s(mv, 2) 
                      +  "\nRI= " + IJ.d2s(ri, 2)
                      +  "\n"+ DEGREES[polyChoice] +" polyn.";
            }
            ...
            plot.addLabel(labelPos, 0.085, label);
